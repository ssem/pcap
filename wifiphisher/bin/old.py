...
def get_internet_interface():
    ...
    inet_iface = None
    proc = Popen(['/bin/ip', 'route'], stdout=PIPE, stderr=DN)
...
def check_dep(prog):
    ...
    p.wait()
    if p.returncode != 0:
        sys.stderr.write('%s: is not installed on your system\n' % prog)
        exit(-1)
...
def parse_args():
    ...
    parser.add_argument("-jI", "--jamminginterface",
    ...
    parser.add_argument("-aI", "--apinterface",
        help="Choose monitor mode interface. By default script will find the most powerful interface and starts monitor mode on it. Example: -i mon5")
...
def parse_conf():
    ...
    for f in ['phiser.conf', '/etc/phiser.conf']:
        if os.path.exists(f):
            conf.read(f)
            if conf.has_option('phiser', 'http_port'):
                http_port = conf.get('phiser', 'http_port')
            if conf.has_option('phiser', 'ssl_port'):
                ssl_port = conf.get('phiser', 'ssl_port')
            if conf.has_option('phiser', 'pem'):
                pem = conf.get('phiser', 'pem')
            if conf.has_option('phiser', 'phising_page'):
                phising_page = conf.get('phiser', 'phising_page')
            if conf.has_option('phiser', 'monitor_iface'):
                monitor_iface = conf.get('phiser', 'monitor_iface')
            if conf.has_option('phiser', 'internet_iface'):
                internet_iface = conf.get('phiser', 'internet_iface')
    return http_port, ssl_port, pem, phising_page, monitor_iface, internet_iface
    ...
    while hop_daemon_running:
        try:
            ...
            for line in iw.communicate()[1].split('\n'):
                if len(line) > 2:
                    with lock:
                        err = '[%s-%s] Channel hopping failed' % (R, W)
                    break
            time.sleep(1)
        except KeyboardInterrupt:
            sys.exit()
...
if __name__ == "__main__":
    ...
    args = parse_args()
    # Parse conf
    #http_port, ssl_port, pem, phising_page, mon_iface, inet_iface = parse_conf()
    # Start HTTP server in a background thread
    #Handler = HTTPRequestHandler
    #httpd = HTTPServer(("", int(http_port)), Handler)
    #sys.stdout.write('[%s*%s] Starting HTTP server at port %s\n' % (T, W, http_port))
    #webserver = Thread(target=httpd.serve_forever)
    #webserver.daemon = True
    #webserver.start()
    # Start HTTPS server in a background thread
    #Handler = SecureHTTPRequestHandler
    #httpd = SecureHTTPServer(("", int(ssl_port)), Handler)
    #sys.stdout.write('[%s*%s] Starting HTTPS server at port %s\n' % (T, W, ssl_port))
    ...
    #os.system('iptables -t nat -A PREROUTING -p tcp --dport 80 -j DNAT --to-destination 10.0.0.1:%s' % http_port)
